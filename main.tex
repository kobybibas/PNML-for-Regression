...
%{\normalsize }The pNML is not only a robust learning scheme but also provides a learnability measure for the specific test sample and the specific training set, for a given hypothesis class. 
...
%the data and labels are generated from a probabilistic source, i.e. there is probabilistic assignment $P_\theta(y|x)$ from a set of hypothesis class $\theta \in \Theta$
...
%of the way the pairs $\{x,y\}$ are generated are the stochastic, the 
...
%  the Empirical Risk Minimization (ERM) \cite{vapnik1992principles}. 
...
%\textbf{Universal Prediction and Online Learning.}
%On-line learning is the case where prediction is done in an on-line fashion on a sequence of test data, accumulated the loss and continues after the corresponding label is revealed. This problem is an extension of the well studied work on universal prediction, see the comprehensive survey \cite{universal_prediction}, which is essentially an online learning but with no $x$'s. As described in \cite{universal_prediction}, the universal prediction solution with log-loss provides a ``universal probability'' for the entire sequence, i.e., $q(y^N)$, which can be converted to a sequential prediction strategy via the chain rule $q(y^N)=\prod_{t=1}^N q(y_t|y^{t-1})$. The universal probability is given by either a Bayesian mixture $q(y^N) = \int_\Theta w(\theta) p_\theta (y^N)$ (in the stochastic setting) or by the normalized maximum likelihood (NML), $q_{\mbox{\tiny{NML}}}(y^N) = \frac{\max_{\theta} p_{\theta}(y^N)}{\sum_{\tilde{y}^N}\max_{\theta} p_{\theta}(\tilde{y}^N)}$,  \cite{shtar1987universal}, (in the individual setting). Both solutions solve a corresponding minmax problem. So universal prediction is well understood.

%Online learning, with a feature sequence $x^N$ is less understood as it has one important difference: the chain rule does not hold for conditional probabilities. Yet in \cite{Fogel2017} a universal learning solution is proposed, which is close to the optimal solution for certain classes of models.
...
% as the normalization factor of the pNML learner
...
% and $e \in R^{N+1}$ for the noise vector
\begin{equation}
...
%$P_N$ is the inverse covariance matrix of the data.§
...
% exp\left\{-\frac{1}{2\sigma^2}\left((1-x^T P_{N+1} x)y-\hat{y} + x^T P_{N+1} x\hat{y} \right)^2\right\}  \\
% =\frac{1}{\sqrt[]{2\pi\sigma^2}}
% exp\left\{-\frac{1}{2\sigma^2}\left((1-x^T P_N x)y-(1 - x^T P_N x )\hat{y} \right)^2\right\}  \\
...
%f least squares with Tikhonov regularization hypothesis class is
\begin{equation}
...
%Applying the singular value decomposition (SVD) on the training set 
...
%where M is the number of features ($x \in R^M$) and 
...
\end{equation}
%Note that $u_i^T x$ is the projection of $x$ on the i-th dimension of the correlation matrix of the training set. 
...
%, while if the test sample is in a subspace corresponding to small (or zero) eigenvalues, the regret will be large.
% and hence the model prediction is not reliable.
...
%The mean of that pNML prediction solution is ERM solution, however, using the pNML, training set based uncertainty measure is obtained.
...
%This work suggests a number of potential directions for future work. These include: (1) evaluate the pNML learners for more hypothesis classes. (2) Obtain the learnable space under the PAC and Stochastic setting.
